...
pub fn part1(input: &[u32]) -> usize {
    ...
    for ea in input {
        let fuel_cost : usize = input.into_iter().map(|&x| {
            (*ea as i32 - x as i32).abs() as usize
        }).sum();
...
pub fn part1_sorted(input: &[u32]) -> usize {
    ...
    sorted_input.into_iter().map(|x| {
        (x as i32 - middle_location as i32).abs() as usize
    }).sum()
...
pub fn part2(input: &[u32]) -> usize {
    ...
    let average_value_high = (input.into_iter().sum::<u32>() as f32 / input.len() as f32).ceil() as usize;
    let average_value_low = average_value_high - 1;

    let avh = input.into_iter().map(|&x| {
        let distance = (x as i32 - average_value_high as i32).abs() as usize;

        (distance * (distance + 1)) / 2
    }).sum::<usize>();

    let avl = input.into_iter().map(|&x| {

...
pub fn input_generator(input: &str) -> Polymerization {
    ...
    let polymer_template = split_input.next().expect("Starting polymer template does not exist in input");

    let mut rules = FnvHashMap::default();
    split_input.next().expect("Pair insertion rules do not exist in input").lines().into_iter().for_each(|line| {
...
pub fn part1(input: &Polymerization) -> usize {
    ...
    input.polymer_template.as_bytes().windows(2).for_each(|byte_slice| {
        *poly_chain_map_old.entry(byte_slice.to_owned()).or_insert(0) += 1;
    });

    for _ in 0..10 {
        poly_chain_map_old.iter().for_each(|(byte_slice, &count)| {
            let insertion = input.rules.get(byte_slice.as_slice()).expect("Rule does not exist in Polymerization structure");


            *poly_chain_map_new.entry([byte_slice[0], insertion[0]].to_vec()).or_insert(0) += count;
            *poly_chain_map_new.entry([insertion[0], byte_slice[1]].to_vec()).or_insert(0) += count;
        });
    ...
    let first_byte = input.polymer_template.bytes().next().expect("Couldn't get first byte of polymer_template").to_owned();
    ...
    max.expect("Max was never populated with Some!") - min.expect("Min was never populated with Some!")
}
...
pub fn part1_slow(input: &Polymerization) -> usize {
    ...
    for _ in 0..10 {
        ...
        poly_chain = poly_chain.windows(2).map(|byte_slice| {
            let insertion = input.rules.get(byte_slice).expect("Rule does not exist in Polymerization structure");
